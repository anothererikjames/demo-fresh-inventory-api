...
const idField = "sku";
let items = [{"sku":"sku-101","status":"active","name":"Widget Prime","quantity":42,"warehouseId":"wh-west-1","unitPrice":49.99,"createdAt":"2026-07-16T20:00:00Z","updatedAt":"2026-07-16T20:00:00Z"}];
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === resourcePath && req.method === "POST") {
    try {
      ...
      const created = { [idField]: `sku-${Date.now()}`, status: "active", ...input, createdAt: now, updatedAt: now };
      items.push(created);
